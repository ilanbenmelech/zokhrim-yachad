...
function buildCards(pairs) {
  ...
  return [...chosen, ...chosen]
    ...
    .map((emoji, i) => ({ id: i, emoji, flipped: false, matched: false, attempts: 0 }))
}
...
export default function MemoryGame() {
  ...
  const [totalChoices, setTotalChoices] = useState(0) // סך הבחירות
    ...
    <div className="screen-enter flex flex-col items-center min-h-[85vh]">
      <div className="top-bar">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="px-4 py-2 rounded-full border-2 border-accent text-accent font-semibold text-[15px] bg-white active:scale-95" onClick={giveHint}>
            💡 רמז
          </button>
        </div>
      </div>
      ...
      <div className={`grid ${gridCols(PAIRS)} gap-3 w-full max-w-sm my-2`}>
        {cards.map(card => (
          <button key={card.id} onClick={() => flipCard(card)}
            className={`aspect-square rounded-2xl border-3 text-[36px] flex items-center justify-center transition-all duration-200 select-none
